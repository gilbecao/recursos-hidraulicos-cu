<template>
  <div>
    <svg ref="stackedBarChart"></svg>
  </div>
</template>

<script>
import * as d3 from 'd3';

export default {
  mounted() {
    this.renderStackedBarChart();
  },
  methods: {
    renderStackedBarChart() {
      // Datos de ejemplo para el gráfico de barras apiladas
      const data = [
        {
          grupo: "Grupo 1",
          Turbidez: 10.5,
          Temperatura: 25.3,
          Conductividad: 350.2,
          PH: 7.2,
          "Suspensión de Sólidos Totales": 15.8
        },
        {
          grupo: "Grupo 2",
          Turbidez: 9.8,
          Temperatura: 24.5,
          Conductividad: 345.1,
          PH: 7.0,
          "Suspensión de Sólidos Totales": 16.3
        },
        // Agrega más datos aquí...
      ];

      const width = 400;
      const height = 300;
      const margin = { top: 20, right: 30, bottom: 30, left: 40 };

      const svg = d3.select(this.$refs.stackedBarChart)
        .attr('width', width)
        .attr('height', height);

      // Lista de variables en los datos
      const variables = Object.keys(data[0]).slice(1);

      const stack = d3.stack()
        .keys(variables)
        .order(d3.stackOrderNone)
        .offset(d3.stackOffsetNone);

      const series = stack(data);

      const xScale = d3.scaleBand()
        .domain(data.map(d => d.grupo))
        .range([margin.left, width - margin.right])
        .padding(0.1);

      const yScale = d3.scaleLinear()
        .domain([0, d3.max(series, d => d3.max(d, d => d[1]))])
        .nice()
        .range([height - margin.bottom, margin.top]);

      const colorScale = d3.scaleOrdinal()
        .domain(variables)
        .range(d3.schemeCategory10);

      svg.selectAll()
        .data(series)
        .enter().append('g')
        .attr('fill', d => colorScale(d.key))
        .selectAll('rect')
        .data(d => d)
        .enter().append('rect')
        .attr('x', d => xScale(d.data.grupo))
        .attr('y', d => yScale(d[1]))
        .attr('height', d => yScale(d[0]) - yScale(d[1]))
        .attr('width', xScale.bandwidth());

      // Eje X
      svg.append('g')
        .attr('transform', `translate(0,${height - margin.bottom})`)
        .call(d3.axisBottom(xScale));

      // Eje Y
      svg.append('g')
        .attr('transform', `translate(${margin.left},0)`)
        .call(d3.axisLeft(yScale));
    },
  },
};
</script>
